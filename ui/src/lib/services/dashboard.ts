...
export function getMasteryColour(level: MasteryLevel): string {
	switch (level) {
		...
		case "good":
			return "var(--accent-light-green)";
		case "learning":
			return "var(--color-warning)";
		case "difficult":
			return "var(--color-error)";
		case "new":
			return "var(--mastery-new)";
	}
}
...
export async function getDashboardStats(
	...
	const accuracy =
		withReps.length > 0
			? Math.round((accurate / withReps.length) * 100)
			: 0;
...
export async function getCharacterGrid(
	script: string,
): Promise<CharacterGridItem[]> {
...
export interface ScriptStudyState {
	script: string;
	label: string;
	glyfsToLearn: string[]; // glyphIds — lesson-ordered, capped to remaining daily quota
	glyfsToReview: string[]; // glyphIds — due for review right now
	masteryBreakdown: MasteryBreakdown;
}

export async function getMasteryBreakdown(
	script: string,
): Promise<MasteryBreakdown> {
	const characters = await db.characters
		.where("script")
		.equals(script)
		.toArray();
...
export async function getProgressForScript(
	scriptId: string,
): Promise<ScriptProgressItem | null> {
